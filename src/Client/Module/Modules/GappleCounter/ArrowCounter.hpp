...
public:
    ...
    appleCounter() : Module("apple Counter", "Counts how many apples you have\nin your inventory.",
                            IDR_arrow_PNG, "") {
        Module::setup();
    };

    void onEnable() override {
        Listen(this, TickEvent, &appleCounter::onTick)
        Listen(this, RenderEvent, &appleCounter::onRender)
        Module::onEnable();
    }

    void onDisable() override {
        Deafen(this, TickEvent, &appleCounter::onTick)
        Deafen(this, RenderEvent, &appleCounter::onRender)
        Module::onDisable();
    }

    void defaultConfig() override { Module::defaultConfig();
        if (settings.getSettingByName<std::string>("text") == nullptr)
            settings.addSetting("text", (std::string) "{value} apples");
...
"responsivewidth")->value);
                ...
                if (SDK::clientInstance->getLocalPlayer()->getSupplies() != nullptr) {
                    ...
                    if(offhandItem != nullptr)
                        if (offhandItem->getItem() != nullptr)
                            if (offhandItem->getItem()->name == "apple")
                                applesCount = offhandItem->count;
    ...
    void onRender(RenderEvent &event) {
        auto applesStr = FlarialGUI::cached_to_string(apples);
        this->normalRender(13, applesStr);
    }
};